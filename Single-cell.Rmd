---
title: "A short tutorial to help beginners in biostatistics and genomics understand an usual single-cell RNA-sequencing workflow: looking at the voting behavior at the French parliament"
...
.bio_lib <- c("Biobase", "clusterExperiment")
.cran_lib <- c("knitr", "kableExtra", "tidyverse", "RColorBrewer", "matrixStats")
...
MetaData <- voting_record %>% select(circo, dept, name, surname, identifiant, iden,
...
Assay <- ExpressionSet(assayData = voting_record,
                       ...
                       featureData = AnnotatedDataFrame(bills))
```
...
```{r}
sum(exprs(Assay)[,pd@data$name == "Bartolone"])
...
j_opt <- which.min((voting_behavior$n_votes)^2 +
                   (1-voting_behavior$n_votes_cast)^2)
...
```{r}
# Filter
Assay <- Assay[rowSums(exprs(Assay) != 0) >= j_opt, ]
```

```{r clean, echo=F}
rm(MetaData, Nb_Votes, voting_behavior, bills, voting_record, E, fd, pd, j, j_opt)
```
...
DE_Bills <- getBestFeatures(exprs(Assay), groups, contrastType = "Pairs") %>%
...
```{r kable, echo = F}
colnames(contrasts) <- c(cell_spec("LR (right)", color = "#FC8D62"),
                         cell_spec("UDI (center-right)", color = "#E5C494"),
                         cell_spec("GDR (far-left)", color = "#E78AC3"),
...
kable(contrasts, escape = F, align = "llll") %>%
...
We look at the 5 tops genes that differentiate clusters 2 and 4 and we look at the mean vote. For all those votes, nearly everyone in cluster 4 abstained and most in cluster 2 voted for. This is because members of cluster 4 either where in the government at some point, or replaced a delegate that was called into the government. Therefore, they "abstained"" for many votes (because they were not members of parliament at the time).

```{r}
contrasts <- getBestFeatures(ce, contrastType = "Pairs") %>%
             filter(Contrast == "Cl02-Cl04") %>%
             arrange(P.Value) %>%
             top_n(-5, P.Value) %>%
             select(Contrast, Feature) %>%
             mutate(Feature = as.numeric(Feature)) %>%
...
votes <- exprs(Assay) %>% as.data.frame(.) %>%
                          mutate(Vote_Number = featureData(Assay)$Number) %>%
                          filter(Vote_Number %in% contrasts$Feature) %>%
                          as.matrix(.)

clusters <- ce@clusterMatrix[,1]

results <- data.frame(mean2 = rowMeans(votes[,clusters == 2]),
                      sd2 = rowSds(votes[,clusters == 2]),
                      mean4 = rowMeans(votes[,clusters == 4]),
                      sd4 = rowSds(votes[,clusters == 4])) %>%
                    map_df(function(x) round(x, digits = 2)) %>%
                    mutate(cluster2 = paste(mean2, " (", sd2, ")"),
                           cluster4 = paste(mean4, " (", sd4, ")")) %>%
                    select(cluster2, cluster4)

contrasts <- data.frame(contrasts$Title, results)

colnames(contrasts) <- c("Title",
                         cell_spec("Cluster 2\nMean (sd)", color = "#8DA0CB"),
...
By now, you should therefore have the tools to do explore scRNA-seq data on your own. Many other tutorials, either general or focused on one package, exist. A good place to start to find them, or get access to some datasets, is the [website of the bioconductor project](https://bioconductor.org/).
