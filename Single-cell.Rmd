...
We can now look at the votes and ask: which votes set a given group apart from the others? We look at the political labels that we know and find the votes that separate them, to find bills that create a lot of political differentiation.
...
In this example, we will ask: What votes distinguish each group from the governing coalition (SER-RRPD-Greens)?
...
contrasts <- getBestFeatures(exprs(Assay), groups, contrastType = "Pairs") %>%
...
votes <- as.data.frame(exprs(Assay))
votes <- votes[featureData(Assay)$Number %in% contrasts$Feature,]
...
colnames(contrasts) <- c("Title",
                         cell_spec("Cluster 2", color = "#8DA0CB"),
                         cell_spec("Cluster 4", color = "#A6D854"))
...
This tutorila only skimmed the analysis of the dataset and there are many other options to explore: looking at voting records accross times, finding which votes draw the most votes, clustering bills, ... The script used to scrap the data can be found on the [Github page](https://github.com/HectorRDB/NationalAssembly) and can be adapted to enquire further.
