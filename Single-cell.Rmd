...
```{r filtering cells}
Nb_Votes <- data.frame(
  Nb_Votes = colSums(!is.na(exprs(Assay))),
...
colnames(E) <- rownames(pd) <- pData(pd)$identifiant
rownames(E) <- rownames(fd) <- pData(fd)$Number
Assay <- ExpressionSet(assayData = E, phenoData = pd, featureData = fd)

Nb_Votes <- data.frame(
  Nb_Votes = colSums(exprs(Assay) != 0),
  delegate = rownames(phenoData(Assay))
)
...
```{r}
sum(exprs(Assay)[, pData(pd)$name == "Bartolone"])
...
voting_behavior <- data.frame(
  n_votes_cast = rep(0, nrow(Assay)),
...
ce <- mergeClusters(ce, mergeMethod = "adjP", plotInfo = "adjP", cutoff = 0.35,
                    clusterLabel = "Clusters", plot = F)
...
Clusters <- data.frame(
  clusters = clusterMatrix(ce)[, 1],
  x = pca$x[, 1], y = pca$x[, 2],
  group = pData(phenoData(Assay))$group1
)
...
ggplot(Clusters,
  ...
  scale_color_manual(values = c("#66C2A5", "#8DA0CB", "#E78AC3", "#FC8D62",
...
contrasts <- DE_Bills %>%
             left_join(pData(featureData(Assay)),
                       by = c("Feature" = "Number")) %>%
             select(-Feature) %>%
             group_by(Contrast) %>%
